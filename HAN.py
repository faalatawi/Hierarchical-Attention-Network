...
class HierarchicalAttentionNetwork(nn.Module):
    ...
    def forward(self, docs, doc_lengths, sent_lengths):
        ...
        """
        doc_embeds, word_att_weights, sent_att_weights = self.sent_attention(
        ...
        return scores, word_att_weights, sent_att_weights, doc_embeds
...
class SentenceAttention(nn.Module):
    ...
    def __init__(
        ...
        self.gru = nn.GRU(
            2 * word_gru_hidden_dim,
            sent_gru_hidden_dim,
            num_layers=sent_gru_num_layers,
            batch_first=True,
            bidirectional=True,
            dropout=dropout,
        )
        ...
        if use_layer_norm:
            self.layer_norm = nn.LayerNorm(
                2 * sent_gru_hidden_dim, elementwise_affine=True
            )
    ...
    def forward(self, docs, doc_lengths, sent_lengths):
        ...
        # Restore as documents by repadding
        word_att_weights, _ = pad_packed_sequence(
            PackedSequence(word_att_weights, valid_bsz), batch_first=True
        )
...
class WordAttention(nn.Module):
    ...
    def forward(self, sents, sent_lengths):
        ...
        packed_words = pack_padded_sequence(
            sents, lengths=sent_lengths.tolist(), batch_first=True
        )
